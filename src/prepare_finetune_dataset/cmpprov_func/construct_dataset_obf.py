...
def process_file(input):
    ...
    with open(file_path, 'rb') as f:
        ...
        for i in range(len(data)):
            ...
            new_examples['compiler'].append(sample['compiler'].split('-')[-1])
...
if __name__ == "__main__":
    ...
    saveloc = 'workdir/4_prepare_finetune_dataset/obf/cmpprov_func/1_99'
    ...
    for dirpath, dirnames, filenames in os.walk(data_dir):
        files.extend([os.path.join(dirpath, fpth) for fpth in filenames if ('x86_64' in fpth) and ('clang-obfus' in fpth)])
