...
if __name__ == "__main__":
    ...
    LABEL_BYTE = 1
    len_limit = int(512)
    data_dir = 'data/binkit2/x32_instfunc_bound/instbound'
    saveloc = f'workdir/4_prepare_finetune_dataset/x86/instbound/{len_limit}/1_99'

    if '95_5' in saveloc:
        assert saveloc.split('/')[-1] != '95_5', 'need to specify data size'

    categories = ['gcc-4.9.4', 'gcc-5.5.0', 'gcc-9.4.0']
    ...
    for category, files in file_lists.items():
        ...
        selected_files.extend(files[:int(len(files)*0.15)])
