...
def read_file(example):
    with open(example['file'], "r") as f:
        ...
        for i, line in enumerate(f):
            ...
            if len(parts) > 1:
                if parts[1] == 'S':
                    ground_truth = LABEL_START
                elif parts[1] == 'B':
                    ground_truth = LABEL_BYTE
                elif parts[1] == '-':
                    ground_truth = -100
                else:
                    raise Exception('Unknown gt')
            else:
                raise Exception('Broken line')
...
def rebuild_dataset(examples):
    ...
    new_examples = {'chunk':[], 'chunk_label':[], 'chunk_idx':[], 'package':[], 'compiler':[], 'arch':[], 'opti':[], 'bin_name':[]}
...
if __name__ == "__main__":
    ...
    LABEL_BYTE = 1
    len_limit = int(512)
    data_dir = 'data/binkit2/obf_instfunc_bound/instbound'
    saveloc = f'workdir/4_prepare_finetune_dataset/obf/instbound/{len_limit}/1_99'

    if '95_5' in saveloc:
        assert saveloc.split('/')[-1] != '95_5', 'need to specify data size'

    categories = ['all', 'bcf', 'fla', 'sub']
    ...
    for category, files in file_lists.items():
        ...
        selected_files.extend(files[:int(len(files) * 0.05)])
