...
def test(args, loader, model, type_criterion, count_criterion):
    ...
    for step, (x, (y, y_cnt)) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for i in range(args.num_args):
                for opti, arch, pred, label in zip(optis, archs, actual_type_predictions[i], y[:,i]):
                    ...
                    total_predictions_type[opti, arch, label] = total_predictions_type.get((opti, arch, label), 0) + 1

                    if pred == label:
                        correct_predictions_type[opti, arch, label] = correct_predictions_type.get((opti, arch, label), 0) + 1
                    
                    if pred != label:
                        false_positives_type[opti, arch, pred] = false_positives_type.get((opti, arch, pred), 0) + 1
                        false_negatives_type[opti, arch, label] = false_negatives_type.get((opti, arch, label), 0) + 1
                    
            for opti, arch, pred, label in zip(optis, archs, actual_count_predictions, y_cnt):
                pred, label = args.count_list[pred.item()], args.count_list[label.item()]

                total_predictions_count[opti, arch, label] = total_predictions_count.get((opti, arch, label), 0) + 1

                if pred == label:
                    correct_predictions_count[opti, arch, label] = correct_predictions_count.get((opti, arch, label), 0) + 1
                
                if pred != label:
                    false_positives_count[opti, arch, pred] = false_positives_count.get((opti, arch, pred), 0) + 1
                    false_negatives_count[opti, arch, label] = false_negatives_count.get((opti, arch, label), 0) + 1
...
if __name__ == "__main__":
    ...
    if args.dataset_arch == 'x86':
        args.dataset_pt = f'workdir/4_prepare_finetune_dataset/x86/funcsig/{dataset_split}'
        projectname = f"GBME_funcsig_{dataset_split}_{args.backbone.lower()}_{model_size}_x64->x86"
    elif args.dataset_arch == 'all':
        args.dataset_pt = f'workdir/4_prepare_finetune_dataset/all_architecture/funcsig/{dataset_split}'
        projectname = f"GBME_funcsig_{dataset_split}_{args.backbone.lower()}_{model_size}_allarch"
    else:
    ...
    #type_criterion = torch.nn.CrossEntropyLoss(ignore_index=args.label_list.index('NULL'), reduction='sum')
