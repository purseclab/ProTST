...
def test(args, loader, model, word_cluster):
    ...
    for step, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for i, (pred, arch, compiler) in enumerate(zip(preds_string, archs, compilers)):
                with open(args.evaluation_file.format(arch, compiler), 'a+') as f:
    ...
    for architecture in args.architecture_list:
        for compiler in args.compiler_list:
            ...
            with open(args.evaluation_file.format(architecture, compiler), 'r') as f:
            ...
            metrics[architecture, compiler] = {'Average': {'Precision':precision, 'Recall':recall, 'F1 Score': f1, 'Total':total, 'Accuracy': acc}}
            end_time = time.time()
            total_inference_time += (end_time - start_time)
...
if __name__ == "__main__":
    ...
    if args.dataset_arch == 'obf':
        args.dataset_pt = f'workdir/4_prepare_finetune_dataset/obf/funcname/{dataset_split}'
        projectname = f"GBME_funcname_{dataset_split}_{args.backbone.lower()}_{model_size}_obf"
    else:
    ...
    args.compiler_list = ['all', 'sub', 'bcf', 'fla']
    args.topk = 8
    ...
    for architecture in args.architecture_list:
        for compiler in args.compiler_list:
            with open(args.evaluation_file.format(architecture, compiler), 'w') as f:
                pass
