...
from BIG2015dataset import BIG2015Dataset
from mlwcls import MalwareClassifier
...
def test(args, loader, model, criterion):
    ...
    for step, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for arch, pred, label in zip(archs, z, y):
                ...
                total_predictions[arch, label] = total_predictions.get((arch, label), 0) + 1

                if pred == label:
                    correct_predictions[arch, label] = correct_predictions.get((arch, label), 0) + 1

                if pred != label:
                    false_positives[arch, pred] = false_positives.get((arch, pred), 0) + 1
                    false_negatives[arch, label] = false_negatives.get((arch, label), 0) + 1
...
if __name__ == "__main__":
    ...
    if args.dataset_arch == 'x86':
        args.BIG2015_dataset_dir = f'workdir/4_prepare_finetune_dataset/x86/mlwcls/{dataset_split}'
        projectname = f"GBME_mlwcls_{dataset_split}_{args.backbone.lower()}_{model_size}_x86"
    elif args.dataset_arch == 'all':
        args.BIG2015_dataset_dir = f'workdir/4_prepare_finetune_dataset/all_architecture/mlwcls/{dataset_split}'
        projectname = f"GBME_mlwcls_{dataset_split}_{args.backbone.lower()}_{model_size}_allarch"
    else:
    ...
    name_list = ['mlwcls', args.backbone.lower()]
    if data_size is not None:
        if '95_5' in args.BIG2015_dataset_dir:
            args.BIG2015_dataset_dir = os.path.join(args.BIG2015_dataset_dir, data_size)
        name_list.append('data_size')
        name_list.append(data_size)
    print(f"dataset path is : {args.BIG2015_dataset_dir}")
    ...
    train_dataset = BIG2015Dataset(args, 'train')
    test_dataset = BIG2015Dataset(args, 'test')
    ...
    assert 'mlwcls' in args.model_path, 'dataset and path not compatible'
    if not args.reload:
    ...
    args.dataset_meta = 'src/5_finetune_linear_evaluation/mlwcls/BIG2015meta.csv'
    model = MalwareClassifier(args)
    if args.reload:
        print('Reloading...')
        if '3_pretrain_contrastive_learning' in args.model_path:
            ...
            print(f'Reload from {args.model_path}.....')
            model.load_state_dict(checkpoint['model_state_dict'])
        elif '5_finetune_linear_evaluation' in args.model_path:
            saved_model_pt = os.path.join(args.model_path, "checkpoint_{}.tar".format(args.epoch_num))
            checkpoint = torch.load(saved_model_pt)
            print(f'Continue finetuning from {args.model_path}.....')
            ...
            saved_model_pt = os.path.join(f'workdir/5_finetune_linear_evaluation/{finetune_from}',args.backbone.lower(),model_size,f'{pretrain_split}',data_size,f'checkpoint_{args.epoch_num}.tar')
        ...
        if args.MLM_pretrain:
            if '95_5' in args.model_path:
                ...
                saved_model_pt = os.path.join(f'workdir/4.5_mlm_task',args.backbone.lower(),model_size,f'{pretrain_split}','5k',f'checkpoint_{args.MLM_pretrain_baseline_epoch}.tar')
