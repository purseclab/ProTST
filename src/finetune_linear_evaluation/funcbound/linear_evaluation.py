...
from funcboundcls import FuncboundClassifier
from bapspecdataset import FuncboundDataset
...
key_to_pop = ['opti', 'package', 'platform']
...
def test(args, loader, model, criterion):
    ...
    for step, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for opti, platform, arch, pred, label in zip(optis, platforms, archs, filtered_pred, filtered_true):
                ...
                total_predictions[opti, platform, arch, label] = total_predictions.get((opti, platform, arch, label), 0) + 1

                if pred == label:
                    correct_predictions[opti, platform, arch, label] = correct_predictions.get((opti, platform, arch, label), 0) + 1

                if pred != label:
                    false_positives[opti, platform, arch, pred] = false_positives.get((opti, platform, arch, pred), 0) + 1
                    false_negatives[opti, platform, arch, label] = false_negatives.get((opti, platform, arch, label), 0) + 1
...
if __name__ == "__main__":
    ...
    if args.dataset_arch == 'x86':
        args.dataset_pt = f'workdir/4_prepare_finetune_dataset/x86/funcbound/{args.input_length}/{dataset_split}'
        projectname = f"GBME_funcbound_{dataset_split}_{args.backbone.lower()}_{model_size}_x86_32"
    elif args.dataset_arch == 'all':
        args.dataset_pt = f'workdir/4_prepare_finetune_dataset/all_architecture/funcbound/{args.input_length}/{dataset_split}'
        projectname = f"GBME_funcbound_{dataset_split}_{args.backbone.lower()}_{model_size}_allarch"
    else:
    ...
    name_list = ['funcbound', args.backbone.lower()]
    ...
    train_dataset = FuncboundDataset(args, 'train')
    test_dataset = FuncboundDataset(args, 'test')
    ...
    assert 'funcbound' in args.model_path, 'dataset and path not compatible'
    if not args.reload:
    ...
    model = FuncboundClassifier(args)
    if args.reload:
        print('Reloading...')
        if '3_pretrain_contrastive_learning' in args.model_path:
            ...
            saved_model_pt = os.path.join(f'workdir/5_finetune_linear_evaluation/x86/{finetune_from}',args.backbone.lower(),model_size,f'{pretrain_split}',data_size,f'checkpoint_{args.epoch_num}.tar')
        ...
        if args.MLM_pretrain:
            if '95_5' in args.model_path:
                ...
                saved_model_pt = os.path.join(f'workdir/4.5_mlm_task/x86',args.backbone.lower(),model_size,f'{pretrain_split}','5k',f'checkpoint_{args.MLM_pretrain_baseline_epoch}.tar')
